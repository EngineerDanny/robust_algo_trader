...
print(f"Running task {param_row}...")
...
MAX_K_LABELS = int(param_dict["max_cluster_labels"])
N_CLOSE_PTS = int(param_dict["price_history_length"])
N_PERC_PTS = int(param_dict["num_perceptually_important_points"])
DIST_MEASURE = int(param_dict["distance_measure"])
N_CLUSTERS = int(param_dict["num_clusters"])
ATR_MULTIPLIER = int(param_dict["atr_multiplier"])
ALGORITHM = param_dict["clustering_algorithm"]

START_WINDOW_ITER = 40
MAX_WINDOW_ITER = 70

random_state = int(param_dict["random_seed"])
train_size = int(param_dict["train_period"] * ONE_DAY)
test_size = int(param_dict["test_period"] * ONE_DAY)
    ...
    for index in range(N_CLOSE_PTS, len(sub_df)):
        try:
            ...
            tp = sub_df["close"].iloc[j] + (
                ATR_MULTIPLIER * sub_df["atr"].iloc[j]
            )
            sl = sub_df["close"].iloc[j] - (
                ATR_MULTIPLIER * sub_df["atr"].iloc[j]
            )
            for k in range(index, len(sub_df)):
                if sub_df["close"].iloc[k] >= tp:
                    pips_y_dict["future_return"] = 1
                    break
                elif sub_df["close"].iloc[k] <= sl:
                    pips_y_dict["future_return"] = -1
                    break
                else:
        ...
        try:
            ...
            if pips_y_dict["future_return"] == 0:
                ...
                for k in range(last_test_idx, len(full_df)):
                    if full_df["close"].iloc[k] >= tp:
                        pips_y_dict["future_return"] = 1
                        break
                    elif full_df["close"].iloc[k] <= sl:
                        pips_y_dict["future_return"] = -1
                        break
                    else:
                        pips_y_dict["future_return"] = 0         
...
ohlcv_data["atr"] = talib.ATR(ohlcv_data["high"], ohlcv_data["low"], ohlcv_data["close"], timeperiod=1)
# clip atr to 0.1 and 0.5
ohlcv_data["atr"] = ohlcv_data["atr"].clip(0.00068, 0.00176)
# round it to 6 decimal places
ohlcv_data["atr"] = ohlcv_data["atr"].round(6)
